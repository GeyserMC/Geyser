                ...
                if (changeLogSets.size() == 0) {
                    ...
                    for (int i = 0; i < changeLogSets.size(); i++) {
                        ...
                        for (int j = 0; j < entries.length; j++) {
                            ...
                            def authorName = entry.author.displayName//new ProcessBuilder("sh", "-c", " git show ${entry.commitId} -s --pretty=format:'%an'").redirectErrorStream(true).start().text.trim() // Get the author name from the commit
